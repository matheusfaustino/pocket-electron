...
* Description
*/
angular.module('pocket', ['lrInfiniteScroll', 'selectize'])
.controller('PocketController', function($scope, pocketService){
  ...
  // it's used $scope because the plugin lrInfiniteScroll
  ...
  pocket.addItem = function() {
    ...
    pocketService.retrieveLater('savedItem').then(function() {
      ...
      pocketService.retrieveLater('retrieveItems').then(function(data) {
        ...
        pocket.item_url = pocket.item_title = pocket.item_tags = '';
  ...
  pocket.showWebview = function(item) {
    // loading item URL
...
.directive('bindMouseEnterLeave', function() {
  return {
    restrict: 'A',
    link: function($scope, elem, iAttrs, controller) {
      // set hidden since the start
      $(elem).find('.actions').css('visibility', 'hidden');

      // bind events
      elem.bind('mouseenter', function() {
        $(elem).find('.actions').css('visibility', 'visible');
...
.directive('keyboardNavigation', function() {
  return {
    restrict: 'A',
    link: function(scope, elem, iAttrs, controller) {
      let parentListClass = '.' + elem[0].className.split(' ').join('.');
      // so consegui rodar se eu colocar isso dentro do watch
      var key = $(parentListClass + ' > li').keynavigator({
        activeClass: 'alert-success',
        useCache: false,
        parentScroll: parentListClass,
        scroll: true,
      });

      // console.log();
      scope.$watchCollection(function() { return scope.pocket.sidebarItems }, function(newItem) {
        console.log('reBuild')
        key.keynavigator.reBuild();
      });
    }
  }
});
